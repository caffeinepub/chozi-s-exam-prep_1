...
import type { ExternalBlob as _ExternalBlob, FileMetadata as _FileMetadata, UserProfile as _UserProfile, UserRole as _UserRole, _CaffeineStorageRefillInformation as __CaffeineStorageRefillInformation, _CaffeineStorageRefillResult as __CaffeineStorageRefillResult } from "./declarations/backend.did.d.ts";
export class Backend implements backendInterface {
    ...
    async addFileMetadata(arg0: FileMetadata): Promise<bigint> {
        if (this.processError) {
            try {
    ...
    async deleteFileMetadata(arg0: bigint): Promise<boolean> {
        if (this.processError) {
            try {
    ...
    async listAllFileMetadata(): Promise<Array<FileMetadata>> {
        if (this.processError) {
            try {
                const result = await this.actor.listAllFileMetadata();
...
async function from_candid_FileMetadata_n17(_uploadFile: (file: ExternalBlob) => Promise<Uint8Array>, _downloadFile: (file: Uint8Array) => Promise<ExternalBlob>, value: _FileMetadata): Promise<FileMetadata> {
...
async function from_candid_vec_n16(_uploadFile: (file: ExternalBlob) => Promise<Uint8Array>, _downloadFile: (file: Uint8Array) => Promise<ExternalBlob>, value: Array<_FileMetadata>): Promise<Array<FileMetadata>> {
    return await Promise.all(value.map(async (x)=>await from_candid_FileMetadata_n17(_uploadFile, _downloadFile, x)));
}
